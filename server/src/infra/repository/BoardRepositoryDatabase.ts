...
export default class BoardRepositoryDatabase implements BoardRepository {
	...
	async findAll(): Promise<Board[]> {
		const boardsData = await this.connection.query(
			"select id_board, name from thiago.board",
			[]
		);
		const boards: Board[] = [];
	...
	async get(idBoard: number): Promise<Board> {
		const [boardData] = await this.connection.query(
			"select * from thiago.board where id_board = $1",
			[idBoard]
		);
	...
	async save(board: Board): Promise<number> {
		const [boardData] = await this.connection.query(
			"insert into thiago.board (name) values ($1) returning *",
			[board.name]
		);
	...
	async update(board: Board): Promise<void> {
		await this.connection.query(
			"update thiago.board set name = $1 where id_board = $2",
	...
	async delete(idBoard: number): Promise<void> {
		await this.connection.query(
			"delete from thiago.board where id_board = $1",
			[idBoard]
		);
	}
}
