...
export default class CardRepositoryDatabase implements CardRepository {
    ...
    async save(card: Card): Promise<number> {
		const [cardData] = await this.connection.query("insert into thiago.card (id_column, title, estimative) values ($1, $2, $3) returning *", [card.idColumn, card.title, card.estimative]);
		return cardData.id_card;
	}

	async update(card: Card): Promise<void> {
		await this.connection.query("update thiago.card set title = $1, estimative = $2 where id_card = $3", [card.title, card.estimative, card.idCard]);
	}
	
	async delete(idCard: number): Promise<void> {
		await this.connection.query("delete from thiago.card where id_card = $1", [idCard]);
	}

	async updateIdColumnAndIndex(idCard: number, idColumn: number, index: number): Promise<void> {
		await this.connection.query("update thiago.card set id_column = $1, index = $2 where id_card = $3", [idColumn, index, idCard]);
	}

	list (title: string): Promise<Card[]> {
		if (!title) {
			return this.connection.query("select * from thiago.card", []);
		} else {
			return this.connection.query("select * from thiago.card where title like $1", [`%${title}%`]);
		}
	}

	async get(idCard: number): Promise<Card> {
		const [cardData] = await this.connection.query("select * from thiago.card where id_card = $1", [idCard]);
